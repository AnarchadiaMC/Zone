...
- **Zero-Touch Asset Provisioning**: When `ZoneClient.dll` is injected, it automatically provisions necessary configuration (`mod_system_zone_online.ltx` via DLTX, only if missing) and syncs scripts + UI layouts (`zone_ui_server_list.xml`, overwritten atomically to match the DLL).
...
Copy `ZoneClient.dll` (+ `ZoneClient_Injector.exe`) to the game `bin` directory (next to `AnomalyDX11.exe`), then inject BEFORE opening the Zone browser in-game.
